...
const CMNews = () => {
  ...
  useEffect(() => {
    ...
    const fetchNewsData = async () => {
      try {
        ...
        setError("डेटा प्राप्त करने में त्रुटि। कृपया बाद में पुनः प्रयास करें।");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="pt-3 text-center" style={{ backgroundColor: "rgb(244, 248, 248)" }}>
        <h3>लोड हो रहा है...</h3>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="pt-3 text-center text-danger" style={{ backgroundColor: "rgb(244, 248, 248)" }}>
        <h3>{error}</h3>
      </div>
    );
  }
  ...
  return (
    <div className="pt-3" style={{ backgroundColor: "rgb(244, 248, 248)" }}>
      <div className="text-center mb-3">
        <h3 className="fw-bold bi-newspaper">&nbsp;मुख्यमंत्री समाचार</h3>
      </div>

      <div className="container-fluid">
        <div className="row g-4">
          {cmNewsData.slice(0, 4).map((news, index) => (
            <div className="col-md-6" key={index}>
              <div className="card shadow-lg rounded-2 h-100">
                <div className="d-flex flex-column flex-md-row bg-white overflow-hidden rounded-2 h-100">
                  <img
                    src={news.featured_photo_hindi && news.featured_photo_hindi !== "#" ? news.featured_photo_hindi : defaultImg}
                    onError={(e) => {
                  ...
                  <div className="p-3 d-flex flex-column justify-content-between">
                    ...
                    <p className="text-muted mb-0" style={{ fontSize: "11px" }}>
                      ...
                      {new Date(news.publish_date).toLocaleDateString("hi-IN", {
                        day: "2-digit",
                        month: "2-digit",
                        year: "numeric",
                      }) +
                        " " +
                        new Date(news.publish_date).toLocaleTimeString("hi-IN", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                    </p>
        ...
        <div className="text-start mb-3 mt-4">
          <div className="text-danger fw-bold text-decoration-none" style={{cursor:"pointer"}} onClick={()=>navigate("/cmnews")}>
            और देखें +
          </div>
        </div>
      </div>
